...
class FunConstrainedOutputModel(Module):
    """A model that has its output constrained.
    ...
    def __init__(self, model: Module, constraint: int, fun, forward_kwargs: Dict[str, Any] = None):
        """Initializes ConstrainedOutputModel."""
        super().__init__()
        self.model = model
        self.constraint = constraint
    ...
    def __call__(self, x: Tensor, *args, **kwargs) -> Tensor:
        ...
        """
        output = self.model(x, *args, **self.forward_kwargs, **kwargs)
        return self.some_fun(output[:, self.constraint])
