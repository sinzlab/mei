...
class ConstrainedOutputModel(Module):
    ...
    def __init__(self, model: Module, constraint: (int, List), target_fn=None, forward_kwargs: Dict[str, Any] = None):
    ...
    def __call__(self, x: Tensor, *args, **kwargs) -> Tensor:
        ...
        return self.target_fn(output) if (not self.constraint and self.constraint != 0) else self.target_fn(output[:, self.constraint])
